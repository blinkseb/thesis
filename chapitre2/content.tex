...
La deuxième partie est le détecteur à micropistes. Englobant le détecteur à pixels, ce détecteur est composé de plus de 10 couches de micropistes en silicone, pour un total d'environ 10 millions de micropistes. On dénombre 4 sous-ensemble :
...
En contrepartie de ces excellentes caractéristiques, les cristaux de tungstate de plomb ont un inconvénient majeur : leur réponse est très sensible à la température (\tilde $\SI[per-mode = symbol]{2}{\percent\per\degreeCelsius}$). Il est donc nécessaire de maintenir une température très sable dans ECAL : le système de refroidissement permet en effet de maintenir la température à $\pm \SI{0.05}{\degreeCelsius}$.
