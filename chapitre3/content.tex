...
En collisionneur hadronique, seuls deux partons interagissent dans l'événement dur. On simule donc un processus $ij \rightarrow X$, où $X$ est une ou plusieurs particules. La section efficace d'un tel processus est donnée par
\begin{align}
...
L'algorithme de reconstruction des jets est complexe. Il arrive que des particules créées lors de l'hadronisation ne soient pas correctement agglomérées dans le jet, souvent parce que la trajectoire de ces particules dévie trop de la trajectoire initiale du quark. Des problèmes de calibrations des sous-détecteurs viennent aussi dégrader la mesure de l'énergie des jets.

Afin de corriger ces problèmes, il est nécessaire d'effectuer toute une série de corrections, présentées en détails dans le \cref{chap:jetmet}.
