...
Les particules colorées issues de la génération de l'événement dur radient des gluons ou des photons. Les gluons peuvent eux mêmes radier des gluons, émettant ainsi des particules en cascade, jusqu'à atteindre une échelle d'énergie fixée par le générateur : c'est la gerbe partonique. Une fois cette échelle atteinte, les gluons restants sont forcés de se désintégrer.

Des hadrons non colorés, c'est-à-dire neutres du point de vue de l'interaction forte, sont produits grâce à un processus appelé hadronisation. Ce phénomène se produit environ \SI{5e-24}{\s} après la production de la particule. Il n'existe pas de théorie décrivant l'hadronisation, mais plutôt des modèles phénoménologiques. Ces modèles peuvent être classés en deux catégories : les modèles basés sur les cordes de Lund (\emph{Lund string model} \citep{Andersson198331}), et ceux basés sur le modèle d'agglomération hadronique (\emph{cluster hadronisation}, \citep{cluster}). Le premier utilise les flux de couleurs reliant les partons entre eux. Lorsqu'une paire de partons s'éloignent l'un de l'autre, l'énergie augmente, jusqu'à devenir suffisante pour créer une paire \Pquark{}\APquark{}, brisant ainsi le flux de couleur au profit de deux nouveaux flux, reliant les anciennes particules aux nouvelles. Les modèles d'agglomération hadronique utilisent la conservation des nombres quantiques entre les partons et les hadrons (\emph{parton hadron duality}). Des agrégats de partons sont réalisés, neutres du point de vue de la QCD (pré-confinement). Un agrégat est identifié directement comme un hadron si sa masse est proche de celle d'un hadron connu. Si un agrégat n'est pas identifié comme hadron, il est considéré comme étant une résonance entre deux hadrons plus légers. Les particules ainsi formées peuvent être stables ou non. En cas d'instabilité, les particules se désintègrent selon les rapports d'embranchement mesurés expérimentalement.

Ces effets sont très complexes à simuler, puisque à l'échelle d'énergie où ils se produisent, la QCD devient non-perturbative. Des programmes dédiés permettent néanmoins de simuler toute la chaîne d'hadronisation des particules, chacun utilisant des algorithmes spécialisés afin de simuler au mieux le développement de la gerbe partonique et de la chaîne d'hadronisation. Parmi ces programmes, on peut citer \texttt{Pythia} \citep{pythia} et \texttt{Herwig} \citep{Corcella:2000bw}.

\section{Simulation des événements}

On dispose maintenant d'une liste de particules stables qui peuvent atteindre le détecteur. Avant de pouvoir comparer les données collectées aux prédictions théoriques, il est nécessaire de simuler la réponse de ce dernier. La simulation de la réponse inclue :
\begin{itemize}
    \item Le passage des particules dans le détecteur.
    \item Les courbures des trajectoires des particules chargées dans le champ magnétique.
    \item Les gerbes électromagnétiques et hadroniques.
    \item Les interactions particules - matière.
    \item La réponse électrique des détecteurs.
    \item La désintégration des particules avec un temps de vie qui leur permet de se désintégrer dans le détecteur.
\end{itemize}

Deux approches sont possibles : une simulation détaillée, basée sur \texttt{Geant\;4} \citep{Agostinelli2003250}, mais très coûteuse en temps de calcul, et une moins détaillée, mais extrêmement plus rapide. Ces deux méthodes sont décrites rapidement ci-dessous.
...
Une solution de simulation plus rapide existe au sein de la collaboration. Développée spécialement pour CMS \citep{1742-6596-219-3-032053}, cette simulation utilise une géométrie simplifiée du détecteur, et considère que les matériaux des sous-détecteurs ont une densité moyenne dont l'évolution est paramétrée. La simulation des interactions au sein des divers sous-détecteurs est approximée en se basant sur les simulations complètes du détecteur. Tout en étant beaucoup plus rapide (environ \SI{200}{\ms} par événement), cette simulation permet de reproduire la plupart des observations avec une erreur de l'ordre du pourcent. Elle est très utilisée pour les études de mise-à-jour des détecteurs, où il est nécessaire de pouvoir simuler très rapidement l'effet d'une modification du détecteur sur les observables physiques.

\section{Reconstruction des événements}

La reconstruction d'un événement est effectuée de façon identique lors de la simulation ou d'une prise de données réelle. En effet, la simulation du détecteur fournit en sortie les signaux électriques des sous-détecteurs, de façon identique à ce qui se passe lors des vraies collisions.

\subsection{L'algorithme \emph{Particle-Flow} (PF)}

CMS utilise un algorithme spécialisé pour reconstruire les objets physiques : l'algorithme du \emph{particle-flow} \citep{pf,cms_pf_2,cms_pf_jets,cms_pf_leptons}. En utilisant l'information de tous les sous-détecteurs pour reconstruire l'événement, cet algorithme permet d'améliorer significativement la résolution des particules reconstruites, et s'avère être un moyen efficace de contrebalancer les faiblesses du HCAL \fxerror{Mentionné ceci dans le chapitre 2} : on peut en effet utiliser l'excellente performance du trajectographe pour aider à la reconstruction des hadrons chargés.
...
Avant l'étape suivante, les \emph{hits} associés sans aucune ambiguïté à une trajectoire sont enlevés de la liste des \emph{hits} disponibles, et on réitère ensuite la même procédure en relâchant un peu les contraintes, ce qui permet d'améliorer l'efficacité de reconstruction, tout en gardant un taux de faux très faible grâce à la réduction du nombre de combinaisons. Après la troisième itération, on obtient une efficacité de reconstruction de \SI{99.5}{\%} pour les muons isolés et de plus de \SI{90}{\%} pour les hadrons chargés. Pour les quatrième et cinquième itérations, la contrainte sur le point d'interaction est relâchée, ce qui permet de reconstruire les trajectoires des hadrons chargés secondaires ayant un vertex d'interaction déplacé.
...
\subsubsection{Vertex d'interaction : reconstruction et \pu}

\begin{figure}[tbp] \centering
    \subcaptionbox{\label{fig:res_pv_x}}[0.32\textwidth]{\includegraphics[width=0.32\textwidth]{chapitre3/figs/ResXpaper.pdf}}
    \subcaptionbox{\label{fig:res_pv_y}}[0.32\textwidth]{\includegraphics[width=0.32\textwidth]{chapitre3/figs/ResYpaper.pdf}}
    \subcaptionbox{\label{fig:res_pv_z}}[0.32\textwidth]{\includegraphics[width=0.32\textwidth]{chapitre3/figs/ResZpaper.pdf}}
    \caption{Comparaison entre la résolution de la reconstruction du vertex primaire sur les données 2010 et sur la simulation, selon l'axe $x$ (\subref{fig:res_pv_x}), selon l'axe $y$ (\subref{fig:res_pv_y}) et selon l'axe $z$ (\subref{fig:res_pv_z}), en fonction du nombre de traces.}
    \label{fig:res_pv}
\end{figure}

La reconstruction des vertex d'interaction utilise les traces précédemment reconstruites. Deux étapes sont nécessaires :
\begin{itemize}
  \item Les traces sont agglomérées en "candidats vertex" si la différence entre leurs coordonnées $z$ au point le plus proche de l'axe du faisceau est inférieure à \SI{2}{\mm}.
  \item Un algorithme d'ajustement \citep{Frühwirth:1027031} itère sur tous les agrégats de traces et calcule la position du vertex d'interaction, ainsi que des indicateurs sur la qualité de l'ajustement ($\chi^2$). Lors de l'ajustement, des poids variant entre 0 et 1 sont assignés aux traces de l'agrégat, suivant leur proximité avec l'axe du faisceau.
\end{itemize}

À cause de la présence de \pu, plusieurs vertex d'interaction vont être reconstruits, distribués le long de l'axe du faisceau. Parmi ces vertex, le vertex primaire est identifié comme celui dont la somme quadratique de l'impulsion transverse des traces qui le composent est la plus grande. La \cref{fig:res_pv} présente la résolution de la reconstruction du vertex primaire, selon l'axe $x$, $y$ et $z$.

\medskip

On peut utiliser les autres vertex afin de réduire l'impact du \pu sur les analyses de physiques. Il est en effet possible de savoir si un hadron chargé provient de la collision principale ou non, selon le vertex d'interaction auquel sa trace est attachée. On identifie ainsi tous les hadrons chargés provenant d'un vertex secondaire comme du \pu. Ces hadrons sont alors supprimés du processus de reconstruction de l'événement.

\subsubsection{Agglomération calorimétrique}

L'algorithme d'agglomération calorimétrique de CMS cherche à atteindre 4 buts :
\begin{enumerate}
  \item \begin{sloppypar}Mesurer l'énergie et la direction des particules neutres (photons, hadrons neutres).\end{sloppypar}
...
De façon similaire, un lien entre les deux calorimètres est créé lorsque la position de l'agrégat formé dans le calorimètre électromagnétique est contenue dans la zone limitée par l'agrégat formé dans le calorimètre hadronique. L'enveloppe des agrégats peut ici aussi être agrandie d'une unité de chaque côté. La distance du lien est donnée par $\Delta R$. %dans le plan ($\eta$, $\phi$).
...
Pour chaque muon reconstruit, on enlève les blocs \emph{particle-flow} correspondants de la liste des blocs disponibles, et on procède ensuite à la reconstruction des électrons. On présente \cref{fig:dimu_mass_spectrum} les performances de reconstruction des muons à l'aide du \pf. La résolution obtenue sur la masse reconstruite du \PJpsi est mesurée à $\sigma = \SI{45}{\MeV}$ (\cref{fig:dimu_jpsi}). On peut aussi voir \cref{fig:dimu_mass} le spectre de masse invariante di-muons reconstruit à l'aide des données 2011. L'excellente performance de la reconstruction permet de reconstruire les résonances di-muons de 1 à \SI{100}{\GeV}.
...
Enfin, les blocs \emph{particle-flow} restants n'ont aucune trace liée. On reconstruit avec les agrégats dans le ECAL les photons \emph{particle-flow}, et avec ceux dans le HCAL les hadrons neutres \emph{particle-flow}.
...
\end{align*}
On considère un électron comme isolé si $I_{\Pe}^\text{corrigé} < \SI{10}{\%}$. Cette valeur est plus faible que pour les muons en raison de la taille du cône plus faible également.
...
L'algorithme cherche la valeur minimum $d_{min}$ entre tous les $d_{ij}$ et $d_{iB}$. Si le minimum est obtenu pour une valeur de $d_{ij}$, les particules $i$ et $j$ sont fusionnées (leurs quadrivecteurs sont sommés), et l'algorithme recommence. Au contraire, si le minimum est obtenu pour $d_iB$, la particule $i$ (souvent résultant de la fusion de plusieurs autres particules lors des itérations précédentes de l'algorithme) est considérée comme un jet et enlevée de la liste des particules. L'algorithme s'arrête quand aucune particule ne reste.
...
\subsubsection{L'algorithme d'identification des jets de \Pbottom} \label{sec:b_tagging}
...
\begin{figure}[tbp]
    ...
    \caption{(\subref{fig:btag_discri}) Sortie de l'analyse multivariée de l'algorithme CSV sur un échantillon enrichi \ttbar. Les hautes valeurs sont enrichies en quark \Pbottom, et les trois flèches correspondent aux points de fonctionnements (lâche, moyen, et dur) défini par la collaboration. (\subref{fig:btag_csv_eff}) Efficacité d'étiqueter un jet de \Pbottom en fonction de la valeur du discriminant. Un point de fonctionnement lâche est plus efficace, mais étiquette aussi plus souvent d'autres jets comme jets de \Pbottom \citep{btag_perf}.}
    \label{fig:btag_perf}
\end{figure}


Les propriétés des hadrons \PB peuvent être utilisées pour identifier les jets qu'ils forment. Ces propriétés incluent, entre autre, leur grande masse et leur grand temps de vie. Plusieurs algorithmes sont disponibles dans CMS permettant d'identifier les jets formés par un quark \Pbottom (\emph{b-tagging}, pour étiquetage des \Pbottom), chacun exploitant une spécificité des hadrons \PB. L'algorithme actuellement utilisé par la collaboration CMS est l'algorithme \emph{Combined Secondary Vertex} (CSV, vertex secondaire combiné) \citep{1748-0221-8-04-P04013}, qui est une combinaison de plusieurs algorithmes.

Cet algorithme exploite toutes les variables connues pour discriminer les jets de \Pbottom, entre autres :
\begin{itemize}
  \item Une quantité importante est le paramètre d'impact, définit comme la distance entre le vertex primaire et le point de la trace le plus proche du vertex primaire dans le plan transverse. Les jets de \Pbottom tendent à avoir un paramètre d'impact plus grand que les autres jets.
  \item À cause de leur grand temps de vie, les hadrons \PB vont se déplacer avant de se désintégrer, donnant lieu à un vertex secondaire qui n'est pas dans l'axe du faisceau. La présence d'un tel vertex est un fort indicateur de la présence d'un jet de \Pbottom.
  \item Des variables cinématiques liées au vertex secondaire sont aussi utilisées, comme la masse du vertex, le temps de vol, le nombre de traces associées au vertex ou le paramètre d'impact de la première trace qui donne une masse supérieure à celle du quark \Pcharm.
\end{itemize}

Ces variables sont utilisées dans une analyse multivariée (MVA) qui permet ensuite de fournir un discriminant. Trois points de fonctionnement sont définis, chacun augmentant l'efficacité de correctement étiqueter un jet de \Pbottom, mais au détriment d'un taux de faux plus important (voir \cref{fig:btag_perf}). Pour le point de fonctionnement moyen, l'efficacité de bien étiqueter un jet de \Pbottom est \tilde\SI{70}{\%}, tandis que la probabilité d'étiqueter comme \Pbottom un jet de \Pcharm est \tilde{20}{\%} et \tilde{1}{\%} pour une autre saveur.

\medskip

L'efficacité de l'algorithme sur les données est évaluée en utilisant des jets contenant un muon, provenant de la désintégration du \Pbottom. En effet, ces muons ont tendance à avoir une plus grande impulsion transverse que les muons provenant d'autres processus. De même, le paramètre d'impact de la trace du muon est significativement plus grand lorsque le muon provient de la désintégration d'un \Pbottom. Ces variables sont utilisées pour construire un lot d'événements enrichie en jets de \Pbottom.

\medskip

Afin de prendre en compte les différences de performances de l'algorithme d'étiquetage des jets de \Pbottom entre les données et la simulation, des facteurs de corrections sont appliqués à la simulation, qui dépendent de la valeur du jet étiqueté comme \Pbottom. Pour l'algorithme CSV, au point de fonctionnement moyen, ces facteurs de corrections varient entre \SI{5}{\percent} pour les jets correctement étiquetés et \SI{17}{\percent} pour les jets légers incorrectement étiquetés \Pbottom.

\subsection{L'énergie transverse manquante} \label{sec:met}

Par définition, les collisions au sein de CMS se déroulent le long de l'axe $z$. L'impulsion dans le plan ($x$, $y$) est donc nulle. Par conservation de l'impulsion, ce bilan doit rester nul après reconstruction de l'événement : un bilan non nul signifie que des particules n'ont pas été correctement reconstruites. C'est typiquement le cas des neutrinos, qui n'interagissent que très peu avec la matière, et ne sont par conséquent jamais détectés.
